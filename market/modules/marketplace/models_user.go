...
type User struct {
	...
	IsVendor bool `json:"is_vendor" gorm:"index"`
...
func (u User) CheckPassword(password string) bool {
	return u.PasswordHash == util.PasswordHashV1(u.Username, password)
}

func (u User) CheckPin(pin string) bool {
	return u.PinHash == util.PasswordHashV1(u.Username, pin)
}
...
func CreateUser(username string, password string, pin string, phrase string, inviteCode string) (*User, error) {
	...
	user = &User{
		...
		PasswordHash:   util.PasswordHashV1(username, password),
		PinHash: util.PasswordHashV1(username, pin),
		Username:         username,
		Phrase:	phrase,
...
func CountVendors(dt *time.Time) int {
	var count int
	q := database.Table("users").Where("is_vendor=?", true)
	if dt != nil {
		q = q.Where("registration_date < ?", dt)
	}
	q.Count(&count)
...
func (s User) ViewUser() ViewUser {
	viewUser := ViewUser{
		User:                &s,
		LastLoginDateStr:    util.HumanizeTime(s.LastLoginDate),
		LongDescriptionHTML: template.HTML(userHtmlPolicy.Sanitize(string(blackfriday.MarkdownCommon([]byte(s.LongDescription))))),
		RegistrationDateStr: util.HumanizeTime(s.RegistrationDate),
...
func (users Users) ViewUsers() []ViewUser {
	viewUsers := []ViewUser{}
	for _, user := range users {
		viewUsers = append(viewUsers, user.ViewUser())
	}
	return viewUsers
}
