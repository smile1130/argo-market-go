...
type Package struct {
	...
	CategoryUuid	string `json:"category_uuid" gorm:"index"ForeignKey:CategoryUuid;References:Category(Uuid)`
	VendorUuid   string `json:"vendor_uuid" gorm:"index"ForeignKey:VendorUuid;References:User(Uuid)`
	Description string `json:"description"`
	RefundPolicy string `json:"refund_policy"`
	TotalAmount float64 `json:"total_amount"`
	Measure string `json:"measure"`
	PayMethod string `json:"pay_method"`
	AllowedCrypto string `json:"allowed_crypto"`
	ShippingFrom string `json:"shippinfg_from" gorm:"index"`
	ShippingTo   string `json:"shipping_to" gorm:"index"`
...
type PackagePrice struct {
	Uuid     string  `json:"uuid" gorm:"primary_key"`	
	PackageUuid  string  `json:"package_uuid" gorm:"index;ForeignKey:PackageUuid;References:Package(Uuid)"`
	Price    float64 `json:"price"`
	Amount	float64 `json:"amount"`
}

type PackagePrices []PackagePrice
...
type Category struct {
	Uuid         string  `json:"uuid" gorm:"primary_key"`
	Name	string `json:"name"`
	ParentCategoryUuid	string `json:"parent_category_id" gorm:"index;ForeignKey:ParentCategoryUuid;References:Categories(Uuid)"`
}
...
func (p Package) Validate() error {
	...
	if p.Name == "" {
		return errors.New("Package name can't be Null")
	}
...
func (p PackagePrice) Validate() error {
	...
	if p.Price <= 0.0 {
		return errors.New("Package Price can't be Null")
	}

	return nil
}

func (s ShippingMethod) Validate() error {
	if s.Uuid == "" {
		return errors.New("Empty UUID")
	}

	if s.Method == "" || s.Price <= 0.0 || s.Days <= 1 {
		return errors.New("There must be at least one method")
	}
...
func (p Package) SaveToDatabase() error {
	if existing, _ := FindPackageByUuid(p.Uuid); existing == nil {
...
func (s ShippingMethod) Save() error {
	if existing, _ := FindShippingMethodByUuid(s.Uuid); existing == nil {
		return database.Create(&s).Error
	}
	return database.Save(&s).Error
}
...
/*
	Queries
*/
func FindCategoryByUuid(uuid string) Category {
	var category Category
	database.
		First(&category, "uuid = ?", uuid)
...
func GetTopCategories() (Categories, error) {
	var categories []Category
	err := database.
		Where("parent_category_uuid = ?", "").
		Find(&categories).
		Error
...
func (c Category) SubCategories() (Categories, error) {
	var categories []Category
	err := database.
		Where("parent_category_uuid = ?", c.Uuid).
		Find(&categories).
		Error
...
func (p Package) CategoryName() string {
	...
	database.
		First(&category, "uuid = ?", p.CategoryUuid)

	return category.Name
}

func (p Package) VendorName() string {
	var vendor User
	
	database.
		First(&vendor, "uuid = ?", p.VendorUuid)

	return vendor.Username
}

func (p Package) PackagePrices() PackagePrices {
	var packagePrices []PackagePrice

	database.
		Find(&packagePrices, "package_uuid = ?", p.Uuid)
...
func FindPackagesByVendorUuid(uuid string) Packages {
	var packages []Package

	database.Find(&packages).Where("vendor_uuid = ?", uuid)
	return packages
}
...
func FindShippingMethodByUuid(uuid string) (*ShippingMethod, error) {
	var ships_method ShippingMethod
	err := database.
		First(&ships_method, "uuid = ?", uuid).
		Error
	if err != nil {
		return nil, err
	}
	return &ships_method, err
}
