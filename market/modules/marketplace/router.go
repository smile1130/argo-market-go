...
func ConfigureRouter(router *web.Router) *web.Router {
	...
	loggedInRouter.Get("/marketplace/:location", (*Context).ListSerpItems)
	loggedInRouter.Get("/product/:packageUuid", (*Context).ViewPackageDetailGet)
	...
	// Vendor
	vendorRouter := loggedInRouter.Subrouter(Context{}, "/vendor")
	vendorRouter.Middleware((*Context).VendorMiddleware)
	vendorRouter.Get("/dashboard", (*Context).ViewVendorDashboard)
	vendorRouter.Get("/create/physical", (*Context).ViewCreatePhysical)
	vendorRouter.Post("/create/physical-product", (*Context).CreatePhysicalPost)
	vendorRouter.Get("/create/digital", (*Context).ViewCreateDigital)
	...
	// Profile
	loggedInRouter.Get("/notifications/user", (*Context).ViewUserNotificationsGET)
	loggedInRouter.Get("notification/:uuid", (*Context).ViewNotificationDetail)	
	...
	loggedInRouter.Get("/orders/user", (*Context).ViewUserOrdersGET)
	loggedInRouter.Get("/profile/:username", (*Context).ViewUserProfileGET)
	...
	loggedInRouter.Get("/favorites", (*Context).ViewFavoritesGET)
