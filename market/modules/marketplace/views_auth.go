...
import (
	...
	// "argomarket/market/modules/apis"
...
func (c *Context) Login(user User, w web.ResponseWriter, r *web.Request) {
	...
	c.Success = "You have successfully logged in"
...
func (c *Context) RegisterPOST(w web.ResponseWriter, r *web.Request) {
	...
	if r.FormValue("pin") != r.FormValue("confirm_pin") {
		c.Error = "Pin do not match"
		c.RegisterGET(w, r)
		return
	}

	user, validationError := CreateUser(r.FormValue("username"), r.FormValue("password"), r.FormValue("pin"), r.FormValue("phrase"), r.FormValue("invite_code"))
...
func (c *Context) RegisterGET(w web.ResponseWriter, r *web.Request) {
	...
	util.RenderTemplate(w, "register", c)
}
...
func (c *Context) ViewRecoverPOST(w web.ResponseWriter, r *web.Request) {
	...
	case "3":
		...
		user.PasswordHash = util.PasswordHashV1(user.Username, r.FormValue("password"))
		user.Save()
...
func (c *Context) Login1FactorPOST(w web.ResponseWriter, r *web.Request) {
	...
	// isLoginSuccessful := isCaptchaValid && (user != nil) && user.CheckPassword(r.FormValue("password"))
	isLoginSuccessful := (user != nil) && user.CheckPassword(r.FormValue("password"))
	// if !isCaptchaValid {
	// 	c.Error = "Invalid captcha"
	// 	c.LoginGET(w, r)
	// 	return
	...
	if user.TwoFactorAuthentication {
		...
		fmt.Println("Here notifications")
		CreateNotification(user.Uuid, "This User just logged in.", "/settings/user")
		c.Login(*user, w, r)
	}
}
